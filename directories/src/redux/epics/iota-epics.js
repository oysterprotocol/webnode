...
import {
  fullfillPrepareTranfers,
  fullfillAttachToTangle
} from "../actions/iota-actions";
import { prepareTransfers, attachToTangleOverride } from "../../services/iota";
...
const attachToTangleEpics = (action$, store) => {
  return action$.ofType(IOTA_ATTACH_TO_TANGLE).mergeMap(action => {
    const { data } = action.payload;
    return Observable.fromPromise(attachToTangleOverride(data))
      .map(powResults => {
        fullfillAttachToTangle({ powResults });
      })
      .catch(error => Observable.empty());
  });
};
