...
class StorageBootstrap extends Component {
  constructor(props) {
    ...
    const prepareList = storage.exchanges.map(item => item.transactionId);
    ...
    console.log(
      "same list return -1 -> " + this._compareListFn(prepareList, prepareList)
    );
  ...
  _getRandomItemFn(storage) {
    ...
    console.log(
      "StorageItem -> " + storageItem + " -> from -> " + storageTitle
    );
  }
